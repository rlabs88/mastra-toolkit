...
export async function loadProjectWorkflows(projectRoot: string): Promise<ReadonlyMap<string, ProjectWorkflow>> {
  ...
  for (const entry of entries) {
    ...
    const workflow = loaded.default as AnyWorkflow;
    const schemas = workflow as unknown as { inputSchema: object; outputSchema: object };
    ...
    const agentTool = loaded.agentTool === undefined ? undefined : agentToolSchema.parse(loaded.agentTool);
  ...
  return createTool({
    ...
    execute: async (input, context) => {
      ...
      try {
        ...
        if (result.status === "failed") return { runId: run.runId, status: result.status, error: result.error.message };
...
async function compileWorkflow(sourcePath: string): Promise<{ generation: string; outputPath: string }> {
  ...
  const outputDirectory = join(tmpdir(), "mastra-toolkit-workflows");
  ...
  return {
    ...
    setup(builder) {
      builder.onResolve({ filter: /^[^./]|^@/ }, args => {
        ...
        const resolved = resolvePackage(args.path, projectRequire);
...
function resolvePackage(
  ...
  if (packageName === "zod" || packageName.startsWith("zod/") || packageName.startsWith("@mastra/")) {
    return pathToFileURL(toolkitRequire.resolve(packageName)).href;
  }
  try {
