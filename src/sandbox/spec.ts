...
export function findSandboxSpecPath(explicitPath?: string, startDirectory = process.cwd()): string {
  if (explicitPath) return isAbsolute(explicitPath) ? explicitPath : resolve(startDirectory, explicitPath);

  let directory = resolve(startDirectory);
  while (true) {
    const candidate = join(directory, "sandbox.config.json");
    if (existsSync(candidate)) return candidate;
    const parent = dirname(directory);
    if (parent === directory) break;
    directory = parent;
  }
  throw new Error(`Unable to find sandbox.config.json from ${startDirectory}`);
}
