...
`specialistToolAliases` translates a capability vocabulary into published tool IDs. `.github/agents` is shared with GitHub Copilot, whose agent files declare `tools: [read, search, edit]` because Copilot has no notion of `mastra_workspace_read_file`; specialist tool selection fails closed, so without a translation every repository carrying Copilot agents took the whole host runtime down at mount. The host owns the table because the IDs are its own — this package stays free of Code SDK identifiers. A published ID always wins over an alias of the same name, so the table can never shadow a real tool. An alias resolves to whichever of its IDs are actually published, which keeps it correct as the tool set changes and lets a recognized-but-ungranted capability resolve to nothing; a name that is neither a published ID nor a declared alias is still an unknown tool and still fails the generation. Reserved IDs are rejected across the expansion before the availability filter, because a reserved ID is never published and filtering first would turn a misconfigured alias into a silent no-grant.
