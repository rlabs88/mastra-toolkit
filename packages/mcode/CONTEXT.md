...
This package is the reusable adapter for RLabs' extended Mastra Code runtime. Its root interface exposes controller projections, project adapters, runtime lifecycle, and the `mz` project-runtime manager. The MCode projection mounts non-recursive canonical agents in a scope/build mode pair per canonical role — eight modes since Ayra joined Cortex, Flux, and Zen — and exposes exactly that role set through the existing controller's native `subagent` tool. Both counts are derived from the canonical role registry, never written as literals, so adding a role moves them together. The Studio projection additionally registers each canonical top-level agent as a generic Mastra supervisor over the canonical leaves. `McodeRecipeV2` remains a deprecated compatibility alias. Factory is a sibling adapter over the same shared contract. Runtime defaults are resolved once and passed into the contract; request-local identity, workspace, sandbox, browser, and approval values stay in the binding. MCode does not own the executable process or fork upstream source.

`mz` makes Studio the singular owner of the controller, sessions, workspace, mounted project generation, storage, and observability. It canonicalizes the Git root, uses a project-hashed data directory, and treats the versioned loopback runtime descriptor as authoritative over local binding or PID files. Agent, skill, and MCP generations reload in place; a workflow generation raises a typed restart signal so the detached application supervisor can restart the same URL and the remote TUI can rehydrate its persisted session. Local tracing captures inputs and outputs through Mastra storage with sensitive-data filtering while explicitly disabling the cloud exporter.
...
The remote TUI may gain parity for embedded-only control-plane commands and later provide the transport foundation for an ACP bridge. It will continue to consume the canonical role and project contracts rather than developing alternate prompts, tools, or configuration systems.
