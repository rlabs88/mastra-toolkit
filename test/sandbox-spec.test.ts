...
import { findSandboxSpecPath, loadSandboxSpec, parseSandboxSpec } from "../src/sandbox/spec.js";
...
describe("sandbox specification", () => {
  ...
  test("rejects a mutable Docker image tag", () => {
    const specification = structuredClone(loadSandboxSpec(resolve("sandbox.config.json")));
  ...
  test("rejects credentials and runtime authority in a versioned specification", () => {
    const specification = structuredClone(loadSandboxSpec(resolve("sandbox.config.json"))) as unknown as Record<string, unknown>;
  ...
  test("discovers the repository specification from Mastra's public runtime directory", () => {
    expect(findSandboxSpecPath(undefined, resolve("src/mastra/public"))).toBe(resolve("sandbox.config.json"));
  });
});
