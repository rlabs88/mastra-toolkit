...
Local MCode and Factory are composed from the same versioned [`McodeRecipeV1`](docs/mcode-factory-compatibility.md). The executable `command_run` tool is owned by `@rlabs/sandbox`, requires an active workspace sandbox, and is available to canonical agents, native subagents, delegated children, and project specialists. Factory currently reports canonical mode/native-subagent controller construction as `unsupported-upstream`; no dependency patch, second controller, or Mastra fork is used.
