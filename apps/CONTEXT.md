...
This boundary contains three private composition roots. `mcode` exposes the RLabs CLI built on published Mastra Code APIs, `studio` boots the central or local Studio host with canonical supervisor agents, and `factory` boots Mastra Factory. Each exports the shared runtime contract and its host-local projection for diagnostics while controller construction remains singular and host-owned. Reusable behavior remains in workspace packages. Factory delegation remains unavailable rather than emulated while its upstream controller construction has no canonical mode/subagent input.
